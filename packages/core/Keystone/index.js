...
module.exports = class Keystone {
  ...
  getAdminMeta() {
    ...
    const lists = arrayToObject(
      this.listsArray.filter(list => list.access.read),
      'key',
  ...
  getAdminSchema() {
    ...
    const resolvers = {
      ...
      ...objMerge(
        this.listsArray.map(list => list.getAuxiliaryTypeResolvers())
      ),
      ...
      Mutation: {
        ...objMerge(
          this.listsArray.map(i => i.getAuxiliaryMutationResolvers())
        ),
  ...
  async createItems(itemsToCreate) {
    const createItems = data => {
      return resolveAllKeys(
        mapObject(data, (value, list) =>
          Promise.all(value.map(item => this.createItem(list, item)))
        )
      );
    };
